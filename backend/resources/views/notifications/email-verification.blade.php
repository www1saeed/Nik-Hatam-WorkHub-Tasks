...
<div dir="ltr" style="background-color:#f6f9fc; padding:24px 0; font-family:{{ $fontFamily }};">
    ...
    <table dir="{{ $direction }}" align="center" width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px; margin:0 auto; background:#ffffff; border-radius:10px; border:1px solid #e5e7eb;">
        ...
        <tr>
            <td style="padding:0 24px 24px; text-align:{{ $align }}; color:#111827; font-size:14px; line-height:1.7;">
                ...
                @endif
                @if (! empty($thanks))
                    <div style="margin-bottom:12px;">{{ $thanks }}</div>
                @endif

                <div style="margin-bottom:12px;">{{ $intro }}</div>
                <div style="margin-bottom:16px; font-weight:bold;">{{ $codeLine }}</div>
    ...
    <div style="max-width:600px; margin:16px auto 0; color:#6b7280; font-size:12px; text-align:left;">
        If you are having trouble clicking the "{{ $actionText }}" button, copy and paste the URL below into your web browser:
        <div style="color:#2563eb; margin-top:6px;">{{ $actionUrl }}</div>
    </div>
